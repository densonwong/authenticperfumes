...
const mock = vi.hoisted(() => ({ auth:vi.fn(), client:vi.fn(), rpc:vi.fn(), invalidate:vi.fn() }));
...
beforeEach(() => {
  vi.resetAllMocks(); mock.auth.mockResolvedValue({id});
  mock.client.mockReturnValue({ rpc:mock.rpc });
...
describe("catalog bulk endpoint", () => {
  ...
  it("uses the same authenticated-admin gate as existing product mutations", async () => {
    expect((await POST(request({action:"delete_products",ids:[id],preview:true}))).status).toBe(200);
